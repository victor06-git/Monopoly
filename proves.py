...
class Jugador:#Definir class Jugador
    ...
    def move(self, passos, cells):
        if self.a_preso == True:
            return "Jugador a presó, no pot jugar"
        self.remove_from_current_cell(cells)
        self.posicio = (self.posicio + passos) % len(cells)
    
    # Actualizar la nueva casilla
        self.update_cell(cells)
        return None
    def move_to(self, casella, cells):
        if 0 <=  casella < len(cells):
            self.remove_from_current_cell(cells)
            self.posicio = casella
            self.update_cell(cells)
        else:
            return "Casella no existeix"
    def remove_from_current_cell(self, cells):
        current_cell = cells[self.posicio]
        jugador = self.get_jugador_fitxa()
        cells[self.posicio] = current_cell.replace(jugador, "")
    def update_cell(self, cells):
        casella_actual = cells[self.posicio]
        jugador = self.get_jugador_fitxa()

        if not casella_actual:
            cells[self.posicio] = jugador
        else:   
            cells[self.posicio] += jugador
    def get_jugador_fitxa(self):
        if torn_jugador_B():
            return "B"
        elif  torn_jugador_G():
            return "G"
...
def tablero(): #Funció imprimeix tauler
    ...
    texto0 = "Hola el Jugador 34 ha resultado ser el ganador"
    texto1 = ""
    texto2 = ""
    texto3 = ""
    texto4 = ""    
    ...
    print(f"|{cell7:<8}|{texto0                                     }|{cell23:<8}| {info_7}")
